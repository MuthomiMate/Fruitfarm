...
if (isset($_POST['doneadd'])) {
  ...
  require 'includes/pictures.php';
    ...
    //adding product
    $queryaddproduct = "INSERT INTO product(id_category, name, description, price, id_picture, thumbnail, quantity)
    VALUES ('$id_category', '$name', '$description','$price', '$idproduct', '$img_title', '50')";
...
if (isset($_POST['doneedit'])) {
  ...
  //Check if images have been changed before editing
  if($_FILES["picture1"]["name"] != '' || $_FILES["picture2"]["name"] != '' || $_FILES["picture3"]["name"] != '') {
    $path = "../products/";
    $file = $path . basename($_FILES["thumbnail"]["name"]);
    $uploadOk = 1;
    $type = pathinfo($file,PATHINFO_EXTENSION);
  ...
  $idproduct = get_last_id() + 1;
    ...
    $description = addslashes($_POST['description']);
    $img_title = $_POST['thumbnail'];
    $img_titles=$_FILES["thumbnail"]["name"];


    //adding product
    $queryaddproduct = "UPDATE product SET id_category='$id_category', name='$name', description='$description', price='$price', id_picture='$idproduct', thumbnail='$img_title', quantity='$quantity' WHERE id = '{$item}'";
   // var_dump($connection->query($queryaddproduct));
  
  if ($connection->query($queryaddproduct) == TRUE ) {            
            echo "<div class='center-align'>
                   <h5 class='green-text'>Product Updated Successfully</h5>
                   </div><br><br>";
